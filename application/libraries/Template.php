...
class Template {
	...
	{	
		...
		$this->ci->config->load('seo');

		//set group
		$this->_set_group();
		$this->layout = $this->group . '/' . $this->ci->config->item('OCU_layout_folder') . $this->ci->config->item('OCU_default_layout');
	 ...
	 * @return void
	 */
	public function render($layout='', $cache_me=false, $is_single=false) 
	{	
		$this->_mark('Template_Render_start');
		
		$active_controller = $this->ci->router->class;
		$active_method = $this->ci->router->method;
		$active_view = $this->current_view;

		$seo = $this->ci->config->item('seo');
		if (!empty($seo[$active_controller . '-' . $active_method])) {
			$this->set('site_name', $seo[$active_controller . '-' . $active_method]);
		} else {
			$this->set('site_name', $this->ci->config->item('OCU_site_name'));
		}

		$this->set('active_controller', $active_controller);
		$this->set('active_method', $active_method);
		$this->set('active_view', $active_view);
		...
		if ($this->is_ajax() || $is_single === true)
		{
			$layout = $this->group . '/' . $this->ci->config->item('OCU_layout_folder') . $this->ci->config->item('OCU_ajax_layout');
		...
		if($outputold != $output){
			error_log('原来的$outputold:'.$outputold);
			error_log('新的$output:'.$output);
		}
		global $OUT;
	...
	//---------------------------------------------------------------
	private function _set_group() {
		$home = $this->ci->config->item('home_folder');
		$admin = $this->ci->config->item('admin_folder');
